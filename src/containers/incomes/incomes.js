...
export const getPlannedIncomesNext = createSelector(
  [plannedIncomes, getNextPeriod],
  (plannedIncomes, getNextPeriod) => {
    return getPlannedTransactionsForPeriod(plannedIncomes, getNextPeriod)
  }
)
...
export const addIncomeSaga = function* (action) {
  try {
    ...
    const data = yield call([API, API.addIncome], {...payload, committed: []}, transactionsStatus)
    if (transactionsStatus === 'committed') {
      yield put({
